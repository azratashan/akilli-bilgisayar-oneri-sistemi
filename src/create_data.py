...
random.seed(42) # Her çalışmada aynı veri setini üretmesi için

# Sabit özellik listeleri
GPU_TDP = {
...
# Şablonlar: (Marka, Model, [Desteklenen GPU'lar], Ekran, Çözünürlük, Ağırlık, Soğutma Skoru, Pil, Panel, IsGaming, Portlar, NPU)
TEMPLATES = [
...
def generate_laptops():
    ...
    for t in TEMPLATES:
        ...
        for gpu in gpus:
            if gpu in ["RTX 5090", "M5 Max", "M4 Max"]: configs = [(64, 2000), (128, 4000), (64, 4000)]
            elif gpu in ["RTX 5080"]: configs = [(32, 1000), (32, 2000), (64, 2000), (64, 4000)]
            elif gpu in ["RTX 5070", "RTX 5070 Ti", "M5 Pro", "M4 Pro"]: configs = [(16, 512), (16, 1000), (32, 1000), (32, 2000), (48, 1000), (64, 2000)]
            elif gpu in ["RTX 5060", "RTX 5050"]: configs = [(16, 512), (16, 1000), (32, 1000)]
            else: configs = [(16, 512), (24, 1000), (32, 1000)]
            for ram, ssd in configs:
                yil = 2026 if ("50" in gpu or "M5" in gpu) else 2025
                
                # Güç (TDP) ve Performansın doğru orantılı artması için (Grafiklerin diyagonal çizilebilmesi için)
                if "Apple" in marka:
                    variance = random.uniform(0.98, 1.02) # Apple çipleri çok sabit güçte çalışır (Nokta görünümü)
                else:
                    variance = random.uniform(0.75, 1.25) # Intel/AMD/RTX geniş güç aralıklarında yapılandırılır (Çizgi görünümü)
                    
                actual_tdp = int(GPU_TDP[gpu] * variance)
                cpu_skor = min(100, max(50, int(GPU_SCORE[gpu] * variance)))
                
                # Tekli ve Çoklu Çekirdek Puanları (Geekbench tarzı)
                # Dikey sapmayı azaltmak için dar bir uniform aralık seçiyoruz ki çizgiler düzgün görünsün
                ...
                data.append({
                    "Marka": marka, "Model": f"{model_base} ({gpu})", "Yil": yil, "CPU_Skoru": cpu_skor,
                    "CPU_Single_Core": cpu_single, "CPU_Multi_Core": cpu_multi,
                    "GPU_Skoru": GPU_SCORE[gpu],
                    "RAM_GB": ram, "RAM_MHz": ram_mhz, "SSD_GB": ssd, "SSD_Hiz_MBs": ssd_hiz, "Ekran_Boyutu": ekran,
                    "Ekran_Cozunurluk": coz, "Laptop_Agirlik_kg": agirlik, "Adaptor_Agirlik_kg": adaptor, "Sogutma_Skoru": sogutma,
                    "Pil_Video_Saat": pil, "Portlar": port_count, "Port_Detay": port_detay, "Marka_Skoru": marka_skoru,
                    "Ekran_Karti": gpu, "VRAM_GB": GPU_VRAM[gpu], "NPU_Var": npu, "WiFi_7": yil == 2026,
                    "Panel_Kalitesi": panel, "Is_Gaming": is_gaming, "TDP_Watt": actual_tdp
                })
    return data
...
def create_dataset(output_path: str = "laptop_dataset_2025_2026.csv") -> pd.DataFrame:
    ...
    # Karıştırıp indexi sıfırla ki daha gerçekçi görünsün
